...
func TestIndexServesGuidedQuoteUI(t *testing.T) {
	...
	for _, expected := range []string{"Make a market.", "Post a two-sided quote", "Start a practice session", "Choose a lesson", "How to make a market well", "Spread is opportunity, not guaranteed profit.", "Move with inventory", "Turn audit", "Customer orders", "Session tape: earlier customer flow", "immediate-or-cancel", "loadAuditHistory", "auditGeneration", `role="status"`, `step="0.01"`, `onclick="startDefault()"`, "function startDefault()", "sessionEpoch", "mmg.pending_create", "function initialize()"} {
